...
def load_positions() -> dict:
    ...
    if changed:
        ...
        logger.info("[momentum_store] 포지션 포맷 마이그레이션 완료")
    return migrated
...
def add_position(
    ...
    if entry_date in positions[code]:
        ...
        msg = f"chore: S5 트랜치합산 {code} [{entry_date}]"
    else:
        ...
        msg = (f"chore: S5 포지션 추가 {code} {name} @{entry_price:,} [{entry_date}]"
               + (f" (트랜치{n})" if n > 1 else ""))
    ...
    if not tranche.get("early_gain_triggered") and gain >= 0.15:
        ...
        try:
            ...
            if days_held <= 21:
                ...
                changed = True
                logger.info(
                    f"[S5 조기익절트리거] [{code}] {tranche.get('name', '')}  "
