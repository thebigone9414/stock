"""
Darvas Box 전략5 데이터 영속성 레이어

data/darvas_data.json      — 일일 스크리닝 결과 (update_darvas.py 기록)
data/darvas_positions.json — S5 포지션 (strategies/darvas.py 기록)
"""
import json
import os
import subprocess
...
DARVAS_DATA_PATH = Path("data/darvas_data.json")
DARVAS_POS_PATH  = Path("data/darvas_positions.json")
...
def load_data() -> dict:
    if not DARVAS_DATA_PATH.exists():
        return {**_EMPTY_DATA}
    with open(DARVAS_DATA_PATH, "r", encoding="utf-8") as f:
        return json.load(f)


def save_data(data: dict) -> None:
    DARVAS_DATA_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(DARVAS_DATA_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def get_buy_candidates() -> list:
    """all_pass=True인 매수 후보 [(code, info)] 리스트 (volume_ratio 내림차순)"""
    data   = load_data()
    stocks = data.get("stocks", {})
    result = [(code, info) for code, info in stocks.items() if info.get("all_pass")]
    return sorted(result, key=lambda x: x[1].get("volume_ratio", 0), reverse=True)


# ── darvas_positions (포지션) ────────────────────────────────────────────

def load_positions() -> dict:
    if not DARVAS_POS_PATH.exists():
        return {}
    with open(DARVAS_POS_PATH, "r", encoding="utf-8") as f:
        return json.load(f).get("positions", {})


def save_positions(positions: dict) -> None:
    DARVAS_POS_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(DARVAS_POS_PATH, "w", encoding="utf-8") as f:
...
def add_position(
    ...
    if existing and existing.get("quantity", 0) > 0 and existing.get("entry_price", 0) > 0:
        ...
        msg = f"chore: S5 추가매수 {code} (수량 {old_qty}→{new_qty})"
    else:
        positions[code] = {
            "name":                 name,
    ...
    git_commit_push([str(DARVAS_POS_PATH)], msg)
...
def remove_position(code: str) -> None:
    ...
    save_positions(positions)
    git_commit_push([str(DARVAS_POS_PATH)], f"chore: S5 포지션 제거 {code}")


def update_position_peak(code: str, current_price: int, current_date: str) -> None:
    """고점 갱신 + 조기익절 트리거 체크 (21일 이내 +15% → 목표 +25%)"""
    positions = load_positions()
