...
MANUAL_CODES = {"034020", "0190C0", "305720", "031980", "095610"}
...
def mark_half_sold(code: str, entry_date: str) -> None:
    ...
    if code in positions and entry_date in positions[code]:
        ...
        save_positions(positions)
        git_commit_push([str(MANUAL_POS_PATH)], f"chore: 수동 부분익절 플래그 {code} [{entry_date}]")
...
def reduce_quantity(code: str, entry_date: str, sold_qty: int) -> None:
    ...
    if code in positions and entry_date in positions[code]:
        ...
        git_commit_push(
            [str(MANUAL_POS_PATH)],
            f"chore: 수동 부분매도 수량조정 {code} [{entry_date}] {old_qty}→{new_qty}"
        )
