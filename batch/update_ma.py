...
S2_TAKE_PROFIT      = 0.20  # 익절 기본 +20% (러너 모드에서 사실상 도달 불가 — 안전망)
...
S2_TRAIL_STOP_MIN   = 0.10  # 트레일링스탑 활성화 최소 고점 수익률
...
def _notify_daily_summary(
    ...
    if balance:
        ...
        shared_max   = 10 + extra
        s2_n, s3_n, s4_n, s5_n = _count_shared()
        total_shared = s2_n + s3_n + s4_n + s5_n
        slot_line    = (
            f"\n슬롯: S2+S3+S4+S5={total_shared}/{shared_max}개 "
