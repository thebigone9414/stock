...
def _collect_entries(today_str: str, per_slot_budget: int, slots_free: int) -> list:
    """S4→S5→S3→S2 우선순위로 매수 후보 취합

    - 같은 전략 당일 중복만 차단 (이미 오늘 해당 전략으로 매수한 종목)
    - 교차 전략 허용: S2 보유 중인 종목을 S5로 매수 가능
    - 서로 다른 전략 간 종목 중복 제거 없음 (독립 슬롯 사용)
    """
...
def run_decision(account, notifier: Notifier = None) -> None:
    ...
    if effective_free > 0 and per_slot >= 10_000:
        buy_list = _collect_entries(today, per_slot, effective_free)
    else:
        logger.info(
