...
S2/S3/S4 보유 포지션 청산 결정 + S2/S3/S4 매수 후보 취합 → trade_queue.json 저장
...
[매수 우선순위]
  S4(RS높을수록) > S3(score높을수록) > S2(몸통비율높을수록)
  S2+S3+S4 공유 슬롯: 기본 4개 + 자산 증가 시 추가
"""
import sys
...
def _collect_entries(
    ...
    """S4→S3→S2 우선순위로 매수 후보 취합 → 슬롯 내에서 선택"""
    all_candidates: list = []
...
def run_decision(account, notifier: Notifier = None) -> None:
    ...
    logger.info(
        ...
        f"슬롯:{total_shared}/{max_shared}(S2:{s2_n} S3:{s3_n} S4:{s4_n})  "
    ...
    lines  += [f"슬롯:{total_shared}/{max_shared}(S2:{s2_n} S3:{s3_n} S4:{s4_n})"]
