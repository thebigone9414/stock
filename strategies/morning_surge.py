...
class MorningSurgeStrategy:
    ...
    def _buy_phase(self) -> None:
        ...
        # 주문 가능 금액: S1 기본 1슬롯 + S2 비어있는 슬롯 모두 흡수
        # S2 morning(09:00)이 먼저 실행된 후라 get_positions()는 당일 매수 반영 상태
        try:
            balance        = self.account.get_balance()
            cash           = balance.cash
            base_cap       = ma_store.get_base_capital()
            extra          = ma_store.extra_slots(base_cap, balance.total_eval) if base_cap else 0
            s2_total_slots = S2_MAX_POSITIONS + extra
            s2_used        = len(ma_store.get_positions())
            s2_empty       = max(0, s2_total_slots - s2_used)
            s1_slots       = 1 + s2_empty    # S1 기본 1슬롯 + S2 미사용 슬롯 전부
        ...
        if s2_empty > 0:
            slot_info = f"  (S1 {s1_slots}슬롯: 기본1 + S2여유{s2_empty}슬롯 각 20%)"
        else:
            slot_info = ""
        msg = (
            f"[옥동자 매수]\n"
